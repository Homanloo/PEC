...
def tabulate(components, T, EoS_model, node_number):
    """
    Prepare the required data and store it in a tabulated form.

    Args:
        components (list): List of components.
        T (int): Temperature (k).
        EoS_model (str): EoS model (PR or SRK).
        node_number (int): Number calculation nodes.

    Returns:
        data frame: All the calculated data in a data frame format.
    """
    ...
    # Creating the liquid phase mole fractions to iterate over.
    step = 1/node_number
...
def visualize(components, T, EoS_model, node_number):
    """
    Plotting the P-x-y phase diagram.

    Returns:
        fig (matplotlib figure): The diagram.
        df (dataframe): Calculated dataframe.
    """
